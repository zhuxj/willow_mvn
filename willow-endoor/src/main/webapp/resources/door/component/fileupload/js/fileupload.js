...
//    var imageServer = "http://127.0.0.1:8080";
    ...
    $.imageUpload = function (opt) {
        ...
        imageOpt.url = imageServer + "/image/upload.do";//图片服务上传地址
    ...
    $.fileUpload = function (opt) {
        var fileDefault = {
            ...
            failFun:function (data) {
                alert("文件太大，必须小于"+data.maxSize);
            }
        }
        var fileOpt = $.extend(true, fileDefault, opt);
        fileOpt.url ="/file/upload.do";//图片服务上传地址
        var fileUploadObj = new FileUpload(fileOpt);
