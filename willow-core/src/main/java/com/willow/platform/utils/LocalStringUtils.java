...
public class LocalStringUtils extends org.apache.commons.lang.StringUtils {
    ...
    public static List<String> splitToArray(String srcStr, String split) {
        return Arrays.asList(split(srcStr, split));
    }
    ...
    /**
     * @param oriStr 原始字符串，
     * @param length 要截取的纯文本的长度，
     * @param more   如果长度超过，则用后面加more(比如  ....)
     */
    public static String getPlanString(String oriStr, int length, String more) {
        if (isEmpty(oriStr)) {
            return "";
        }
        String _temp = getPlan(oriStr);
        if (length < 0 || _temp.length() < length) {
            return _temp;
        }
        return _temp.substring(0, length) + more;
    }

    public static String getPlan(String str1) {
        String str = "";
        str = str1;
        str = str.replaceAll("</?[^>]+>", "");   //剔出了<html>的标签
        str = str.replaceAll("\\&[a-zA-Z]{1,10};", "");//去除类似&lt; &gt; &nbsp;的字串
        str = str.replace(".", "");
        str = str.replace("\"", "‘");
        str = str.replace("'", "‘");
